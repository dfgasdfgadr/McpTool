...
### 4.3 推荐：按调试流程生成 MCP 工具

当你希望 AI 稳定完成一个真实业务任务时，优先使用「FLOW」工作台：

1. 打开浮动面板，进入「FLOW」
2. 点击「开始录制」，按真实业务顺序操作页面
3. 扩展会记录每一步用户动作，并把随后出现的接口请求归属到对应步骤
4. 将请求分类为「核心 / 支撑 / 噪音 / 待判定」
5. 对确认可用的核心或支撑请求勾选「已验证」
6. 点击「已验证生成 MCP」

通过 FLOW 生成的工具会携带流程来源、步骤顺序、请求分类和验证状态。MCP 工具测试成功/失败后，也会把测试时间、HTTP 状态码和错误信息保存到工具元数据中，便于判断哪些工具适合暴露给 AI 使用。
...
用于快速定位「页面上看到的某个值来自哪个 API 字段」，并只修改该 JSON Path 验证 UI：

1. 在页面上选中与接口返回一致的文本（如用户名、金额、状态文案）
2. 点击选区附近出现的 **「查来源」** 按钮
3. 在「候选来源」弹窗中查看匹配的 JSON Path、接口与 Flow 步骤
4. 选择一条候选，输入新值后点击 **「保存字段 Mock」**
5. 刷新页面或重新触发该接口，确认仅目标字段变化

说明：

- 候选列表基于响应 JSON 的精确/包含/规范化匹配，**不保证唯一正确**，请结合 Flow 与请求详情判断
- 字段 Mock 写入现有 Mock 规则（`response.patches`），可与整响应 Mock 叠加
- 请求列表中带有 **Patch** 标签的规则表示存在字段级 patch
